...
def generate_data(
    ...
    data = []
    if type(audio) == tuple or type(audio) == list:
        num_words = len(audio)
        for _ in range(sample_size):
            length = desired_length // num_words
            data.append(np.concatenate([pad_audio_random(audio[i], length) for i in range(num_words)]))
